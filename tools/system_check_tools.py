    ...
    def run(self, params: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if action == "list":
                ...
                if not final_cmd:
                    return "Error: Please provide a command to run, e.g., {\"action\": \"run\", \"command\": \"uname\"}"
                
                # 获取命令参数
                named = params.get("params") or params.get(final_cmd) or {}
                extra = params.get("extra_args") or []
                host = params.get("host")
                return _execute_category_command(self.category_key, final_cmd, named, extra, host)
            else:
